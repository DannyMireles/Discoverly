    ...
    >
      <div className="space-y-8">
        {!data.configured ? (
          ...
          <Banner title="Sign in required" tone="warning" action={<Link href="/auth"><Button>Go to Auth</Button></Link>}>Create your admin account before onboarding Zenful Cove.</Banner>
        ) : !data.company ? (
          <Banner title="Onboarding is not done" tone="warning" action={<Link href="/company/onboarding"><Button>Start Onboarding</Button></Link>}>Create Zenful Cove and connect your services.</Banner>
          ...
          >
            Create the affiliate record, copy the exact Lodgify promotion name, then mark setup complete after it exists in Lodgify.
          </Banner>
        ) : null}

        <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-5">
          <StatCard label="Affiliate Revenue Driven" value={formatCurrency(summary.revenueDriven)} icon={<DollarSign className="h-5 w-5" />} />
          <StatCard label="Paid Bookings Driven" value={String(summary.paidBookings)} icon={<CalendarCheck2 className="h-5 w-5" />} tone="green" />
          <StatCard label="Pending Payouts" value={formatCurrency(summary.pendingPayouts)} icon={<BadgeDollarSign className="h-5 w-5" />} tone="amber" />
          <StatCard label="Paid This Month" value={formatCurrency(summary.paidThisMonth)} icon={<CreditCard className="h-5 w-5" />} tone="green" />
          <StatCard label="Active Affiliates" value={String(summary.activeAffiliates)} icon={<Users className="h-5 w-5" />} tone="slate" />
        </section>
